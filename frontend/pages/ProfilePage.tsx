...
type HistoryFilter = 'Vše' | 'Dokončeno' | 'V řešení';
...
const ProfilePage: React.FC<ProfilePageProps> = () => {
  ...
  const [historyFilter, setHistoryFilter] = useState<HistoryFilter>('Vše');
  ...
  const tabs = [
    ...
    { id: 'portfolio', label: 'Historie projektů', icon: <div className="w-5 h-5">{BriefcaseIcon}</div> },
    { id: 'achievements', label: 'Úspěchy', icon: <div className="w-5 h-5">{TrophyIcon}</div> }
  ];

  const filteredPortfolio = useMemo(() => {
    if (historyFilter === 'Vše') return CURRENT_USER.portfolio;
    const targetStatus = historyFilter === 'Dokončeno' ? 'Completed' : 'In Progress';
    return CURRENT_USER.portfolio.filter(p => p.status === targetStatus);
  }, [historyFilter]);
      ...
      case 'portfolio':
        return (
          <div className="animate-fade-in">
            <div className="flex items-center gap-2 mb-6 bg-white p-3 rounded-lg shadow-sm border border-gray-100 overflow-x-auto">
              <span className="text-sm font-bold text-gray-500 mr-2 whitespace-nowrap">Filtrovat:</span>
              {(['Vše', 'Dokončeno', 'V řešení'] as HistoryFilter[]).map(filter => (
                <button
                  key={filter}
                  onClick={() => setHistoryFilter(filter)}
                  className={`px-4 py-1.5 text-xs font-bold rounded-full transition-all whitespace-nowrap ${historyFilter === filter
              ...
              <div className="grid grid-cols-1 gap-4">
                {filteredPortfolio.map(project => (
                  <div key={project.id} className="relative group">
                    <div className="absolute -left-2 top-4 w-1 h-12 bg-accent-teal rounded-full opacity-0 group-hover:opacity-100 transition-opacity"></div>
                    <ProjectCard
                      project={project}
                      onSelect={handleProjectSelect}
                      onApply={handleApplyNow}
                    />
                    {/* Role Overlay / Info could be added here if needed */}
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center py-12 bg-white rounded-xl shadow-sm border border-dashed border-gray-300">
                <p className="text-gray-500 font-medium">V této kategorii zatím nemáte žádné projekty.</p>
                <button
                  onClick={() => setHistoryFilter('Vše')}
          ...
          <div className="relative">
            ...
            {settingsOpen && (
              ...
              >
                ...
                <button onClick={logout} className="flex items-center gap-3 w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50">
        ...
        <div className="flex justify-around border-b border-gray-200">
          {tabs.map(tab => (
            <button
              ...
              className={`flex items-center justify-center w-full py-4 text-sm font-bold transition-all duration-200 focus:outline-none ${activeTab === tab.id
                ? 'text-brand-blue border-b-4 border-brand-blue bg-blue-50/50'
