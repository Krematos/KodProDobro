...
const HomePage: React.FC<HomePageProps> = ({ onProjectSelect, onCreateProject }) => {
  ...
  useEffect(() => {
    const loadProjects = async () => {
      try {
        ...
        const frontendProjects = mapBackendProjectsToFrontend(backendProjects);
        setProjects(frontendProjects);
      } catch (err: any) {
        setError(err.message || 'Nepodařilo se načíst projekty');
  ...
  const allOrganizations = useMemo(() => {
    const orgs = projects.map(p => p.organization);
  ...
  const statusOptions: StatusFilter[] = ['Open', 'All'];
  ...
  }, [projects, statusFilter, skillFilter, organizationFilter, selectedTags, searchQuery]);
  ...
  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
        <h3 className="text-xl font-bold text-red-700 mb-2">Chyba při načítání projektů</h3>
        <p className="text-red-600 mb-4">{error}</p>
        <button
          onClick={() => window.location.reload()}
          className="bg-brand-blue text-white px-4 py-2 rounded-lg hover:bg-opacity-90"
        >
          Zkusit znovu
        </button>
      </div>
      ...
      <div className="mb-6 flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-bold text-brand-dark">
            {isAuthenticated && user ? `Vítejte, ${user.username}!` : "Prozkoumejte projekty"}
          </h1>
        ...
        {onCreateProject && (
          <button
            ...
            className="ml-4 flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-accent-teal to-brand-blue text-white rounded-lg font-semibold shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-0.5"
          >
            ...
            <div className="flex flex-wrap gap-2">
              {statusOptions.map(status => (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-bold rounded-full transition-all ${statusFilter === status
            ...
            <div className="flex flex-wrap gap-2">
              {allTags.map(tag => (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all border ${selectedTags.includes(tag)
